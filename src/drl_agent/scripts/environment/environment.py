...
class Environment(Node):
    ...
    def __init__(self):
        ...
        front_threshold = self.collision_threshold  # 从 yaml 读取，通常是 0.35
        side_threshold = 0.15  # 设置侧面允许的最小距离 (根据实际机器人宽度调整)
        
        # 定义正面角度范围 (例如：-45度 到 +45度 使用正面阈值，其余使用侧面阈值)
        front_angle_limit = np.radians(60)

        for bin_range in self.bins:
            # 计算该扇区的平均角度
            avg_angle = (bin_range[0] + bin_range[1]) / 2.0
            
            # 如果角度在正面范围内，使用大阈值；如果是侧面，使用小阈值
            if abs(avg_angle) < front_angle_limit:
                self.collision_thresholds.append(front_threshold)
            else:
                self.collision_thresholds.append(side_threshold)
    ...
    def step_callback(self, request, response):
        ...
        #雷达最小值
        min_index = np.argmin(environment_state) # 找到最近距离的索引
        min_angle = self.bins[min_index][0] + (self.bins[min_index][1] - self.bins[min_index][0])/2 # 粗略计算该扇区角度
        min_angle_deg = np.degrees(min_angle) # 转为度数方便看
        self.get_logger().info(f"Min Dist: {min_laser:.3f} m | Angle: {min_angle_deg:.1f} deg")
        ...
        # 进度 = 上次距离 - 这次距离
        # 结果 > 0 代表靠近了目标， < 0 代表远离了目标
        progress = self.previous_distance - current_distance
        
        # 【必须更新】：把这次的距离记为“上次距离”，供下一步使用
        self.previous_distance = current_distance
        
        # 将 progress 传给 get_reward
    ...
    def reset_callback(self, _, response):
		...
		*****************************************************"""
        if self.train_mode:
            ...
            # position_ok = False
            # # 随机生成朝向 (保持连续角度，如果需要只有 0/90/180/270 度，可改为 np.random.choice)
            # possible_angles = [0.0, np.pi/2, np.pi, -np.pi/2]
            # angle = np.random.choice(possible_angles)
            # 
            # grid_size = 16  # 定义迷宫是 16x16 的网格
            # # 计算地图的总跨度 (例如 8.0 - (-8.0) = 16.0)
            # map_span = self.upper - self.lower
            # # 计算每个格子的大小 (例如 16.0 / 16 = 1.0)
            # cell_size = map_span / grid_size 
            # 
            # while not position_ok:
            #     # 1. 随机选择 X 和 Y 的网格索引 (0 到 15)
            #     grid_idx_x = np.random.randint(0, grid_size)
            #     grid_idx_y = np.random.randint(0, grid_size)
            # 
            #     # 2. 将索引转换为世界坐标系的中心点
            #     # 公式：下边界 + (索引 * 格子长) + 半个格子偏移
            #     start_x = self.lower + (grid_idx_x * cell_size) + (cell_size / 2.0)
            #     start_y = self.lower + (grid_idx_y * cell_size) + (cell_size / 2.0)
            # 
            #     # 3. 检查这个中心点是否在墙壁内
		...
		*****************************************************"""
        self.change_goal()
        self.get_logger().info(f"Generated Start: ({start_x}, {start_y})")
		...
		*****************************************************"""
        ...
        agent_state = self.get_agent_state()
        # 【新增】：重置时，记录当前的初始距离
        self.previous_distance = agent_state[0]
    ...
    def change_goal(self):
        """Places a new goal"""
        if self.train_mode:
            # === 使用 reset_callback 中选中的终点 ===
            self.goal_x = self.current_train_goal["x"]
            self.goal_y = self.current_train_goal["y"]
    ...
    #         while not goal_ok:
    #         # 1. 随机选择网格索引
    #             grid_idx_x = np.random.randint(0, 16)
    #             grid_idx_y = np.random.randint(0, 16)
            
    #             # 2. 转换为坐标中心
    #             cell_size = 1.0
    #             self.goal_x = self.lower + (grid_idx_x * cell_size) + (cell_size / 2.0)
    #             self.goal_y = self.lower + (grid_idx_y * cell_size) + (cell_size / 2.0)
    ...
    def check_collision(self, laser_data):
        """Detect a collision from laser data using dynamic thresholds"""
        done, collision = False, False
        min_laser = min(laser_data) # 保留这个用于计算 reward
        
        # === [修改后的逻辑] 逐个检查每个扇区 ===
        for i, distance in enumerate(laser_data):
            # 获取该扇区对应的阈值
            threshold = self.collision_thresholds[i]
            # 如果任何一个方向的距离小于该方向的特定阈值，判定为碰撞
    ...
    def get_reward(target, collision, action, min_laser, progress):
        ...
        return action_reward + obstacle_reward + progress_reward

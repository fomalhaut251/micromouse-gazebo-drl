...
class TrainTD7(EnvInterface):
    def __init__(self):
        ...
        self.file_name = (
            f"{self.base_file_name}_{datetime.now().strftime('%Y%m%d_%H%M')}"
        )
    ...
    def train_online(self):
        ...
        for t in range(1, self.max_timesteps + 1):
            ...
            if ep_finished or ep_timesteps == self.max_episode_steps:
                ...
                # --- [New] 2. 记录并保存训练奖励 (开始) ---
                train_rewards.append(ep_total_reward)
                # 为了防止丢数据，每回合都覆盖保存一次，文件名加个前缀 "train_"
                np.save(f"{self.results_dir}/train_{self.file_name}", train_rewards)
                # --- [New] 2. 记录并保存训练奖励 (结束) ---
